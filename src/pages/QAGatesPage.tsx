...
export function QAGatesContent() {
  return (
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-tk-grey-lt border-b border-tk-border">
                <th className="text-left px-4 py-3 text-[11px] font-semibold text-tk-grey/70 uppercase tracking-wide w-28">ID</th>
                <th className="text-left px-4 py-3 text-[11px] font-semibold text-tk-grey/70 uppercase tracking-wide">Campaign</th>
                {GATE_COLUMNS.map(col => (
                  <th key={col.id} className="text-center px-3 py-3 text-[11px] font-semibold text-tk-grey/70 uppercase tracking-wide whitespace-nowrap">{col.label}</th>
                ))}
              </tr>
            </thead>
              ...
              {campaigns.map(campaign => {
                ...
                return (
                  <tr key={campaign.id} className="hover:bg-tk-grey-lt/50 transition-colors">
                    <td className="px-4 py-3 font-mono text-[11px] text-tk-blue2 whitespace-nowrap">{campaign.id}</td>
                    <td className="px-4 py-3 max-w-[180px]"><span className="block truncate text-tk-grey font-medium" title={campaign.name}>{campaign.name}</span></td>
                    {GATE_COLUMNS.map(col => {
                      const status = (row[col.id] ?? 'pending') as GateStatus
                      return <td key={col.id} className={`px-3 py-3 text-center ${cellBg(status)}`}><StatusBadge label={status} variant="gate" size="sm" /></td>
                    })}
                  </tr>
                )
              })}
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="section-title text-base leading-tight">
                Brand Guardian Scan &nbsp;·&nbsp;
                <span className="font-mono text-sm font-semibold text-tk-blue2">{brandGuardianScan.campaign}</span>
              </h2>
              <p className="text-xs text-tk-grey/50 mt-0.5">Scanned {brandGuardianScan.scannedAt}</p>
            </div>
          </div>
          <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap flex-shrink-0 ${overallStatusClass(brandGuardianScan.overallStatus)}`}>
            {brandGuardianScan.overallStatus}
          </span>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {brandGuardianScan.checks.map(check => (
            <div key={check.rule} className={`flex items-start gap-3 rounded-lg px-3 py-2.5 border ${check.status === 'pass' ? 'bg-green-50/60 border-green-100' : check.status === 'fail' ? 'bg-red-50/60 border-red-100' : 'bg-amber-50/60 border-amber-100'}`}>
